<# 
  ...
  Starts all services with:
    ...
    * Log files (services write their own logs via env vars)
    * Startup health checks (wait for each port)
    * Crash recovery via watchdog companion
    
  Run directly:  powershell -ExecutionPolicy Bypass -File start-server.ps1
...
function Check-Dependency {
    ...
    # Quick liveness test: try to run with --version (if supported) or just check it's executable
    $test = $null
    try {
...
if (-not (Test-Path "$APPS\python_env\Scripts\docling-serve.exe") -or
    ...
    Write-Output "[PREFLIGHT] WARNING: Python service launchers not found. Run fix-portable.bat first."
}
Write-Output ""
...
function Launch-Service {
    ...
    try {
        ...
        # Do NOT redirect stdout/stderr -- let services write to the console.
        # Services handle their own logging via environment variables.
...
Write-Output ""
Write-Output "  To stop:   powershell -File stop-server.ps1"
Write-Output "  Watchdog:  powershell -File watchdog.ps1"
Write-Output ""
